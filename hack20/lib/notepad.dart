...
import 'package:hack20/provider/buildBloc.dart';
import 'package:hack20/provider/buildModel.dart';
import 'package:provider/provider.dart';

class Notepad extends StatefulWidget {
  final buildBloc bloc;

  const Notepad({Key key, this.bloc}) : super(key: key);


  static Widget create(BuildContext context) {
    return Provider<buildBloc>(
      builder: (_) => buildBloc(),
      dispose: (context, bloc) => bloc.dispose(),
      //it is used to dispose bloc when widget removed from widget tree
      child: Consumer<buildBloc>(builder: (context, bloc, _) => Notepad(bloc: bloc,)), //to access bloc and pass it to constructor
    );
  }
  @override
...
class _NotepadState extends State<Notepad> {
  ...
  Widget build(BuildContext context) {
    return StreamBuilder<buildModel>(
      stream: widget.bloc.modelStream,
      initialData: buildModel(),
      builder: (context, snapshot) {
        final buildModel model=snapshot.data;
        return Center(
            child: Container(
              height: 500,
              width: 600,
          ...
          children: <Widget>[
            Row(
              children: <Widget>[
                ...
                SizedBox(width: 5,),
                Text('Untitled - Notepad',style: TextStyle(fontFamily: 'segio ui'),),
                SizedBox(width: 360 ,),
                Container(
                  height: 20,
                  width: 20,
                ...
                Container(
                  ...
                  width: 20,
                  color:Color.fromRGBO(191, 189, 189, 0.5),child:
                Center(child: Text('[]',style:TextStyle(fontSize: 12,fontFamily: 'segoe ui') ,)),
                ),
                SizedBox(width: 8,),
                GestureDetector(
                  child: Container(
                  ...
                  Center(child: Text('X',style:TextStyle(fontSize: 16,color: Colors.white,fontFamily: 'segoe ui') ,)),
            ...
            Row(
              children: <Widget>[
                InkWell(
                    child: Text('File',style: TextStyle(fontFamily: 'segio ui',fontSize: 12),),
                    onTap: (){
                        setState(() {
                          fileState=true;
                          editState=false;
                ...
                SizedBox(width: 8,),
                InkWell(
                  child: Text('Edit',style: TextStyle(fontFamily: 'segio ui',fontSize: 12),),
                  onTap: (){
                    setState(() {
                      fileState=false;
                      editState=true;
                ...
                SizedBox(width: 8,),
                InkWell(
                  child: Text('Format',style: TextStyle(fontFamily: 'segio ui',fontSize: 12),),
                  onTap: (){
                    setState(() {
                      fileState=false;
                      editState=false;
                ...
                SizedBox(width: 8,),
                InkWell(
                  child: Text('View',style: TextStyle(fontFamily: 'segio ui',fontSize: 12),),
                  onTap: (){
                    setState(() {
                      fileState=false;
                      editState=false;
                ...
                SizedBox(width: 8,),
                InkWell(
                  child: Text('Help',style: TextStyle(fontFamily: 'segio ui',fontSize: 12),),
                  onTap: (){
                    setState(() {
                      fileState=false;
                      editState=false;
  ...
  _buildOptions(String title,String trailing){
    return Row(
      ...
      children: <Widget>[
        Text(title,style: TextStyle(fontSize: 10,fontFamily: 'segoe ui'),),
        Text(trailing,style: TextStyle(fontSize: 10,fontFamily: 'segoe ui'),),
      ],
