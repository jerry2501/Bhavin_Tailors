...
class KurtiWithTrousers extends StatefulWidget {

  @override
  _KurtiWithTrousersState createState() => _KurtiWithTrousersState();
}

class _KurtiWithTrousersState extends State<KurtiWithTrousers> {
  String userId;
  bool state=false;
  bool isUserLiked;
  @override
  ...
  Future<bool> onLikeTapped(bool isLiked,DocumentSnapshot snapshot) async{
    print(isLiked);
    if(isLiked==true){
      await Firestore.instance.collection('collection').document('blouse').collection('ankle_pant_dress').document(snapshot.data['uid']).updateData({'likedBy':FieldValue.arrayRemove([userId])});
    }
    if(isLiked==false){
      await Firestore.instance.collection('collection').document('blouse').collection('ankle_pant_dress').document(snapshot.data['uid']).updateData({'likedBy':FieldValue.arrayUnion([userId])});
    }
