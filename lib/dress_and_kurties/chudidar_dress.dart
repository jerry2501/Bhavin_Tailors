...
class ChudidarDress extends StatefulWidget {

  @override
  _ChudidarDressState createState() => _ChudidarDressState();
}

class _ChudidarDressState extends State<ChudidarDress> {
  String userId;
  bool state=false;
  bool isUserLiked;
  @override
  ...
  Future<bool> onLikeTapped(bool isLiked,DocumentSnapshot snapshot) async{
    print(isLiked);
    if(isLiked==true){
      await Firestore.instance.collection('collection').document('blouse').collection('chudidar_dress').document(snapshot.data['uid']).updateData({'likedBy':FieldValue.arrayRemove([userId])});
    }
    if(isLiked==false){
      await Firestore.instance.collection('collection').document('blouse').collection('chudidar_dress').document(snapshot.data['uid']).updateData({'likedBy':FieldValue.arrayUnion([userId])});
    }
  ...
  Widget build(BuildContext context) {
    return SingleChildScrollView(
      ...
      StreamBuilder<QuerySnapshot>(
          stream: Firestore.instance.collection('collection').document('blouse').collection('chudidar_dress').snapshots(),
