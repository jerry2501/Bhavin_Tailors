...
class DesignerBlouse extends StatefulWidget {

  @override
  _DesignerBlouseState createState() => _DesignerBlouseState();
}

class _DesignerBlouseState extends State<DesignerBlouse> {
  String userId;
  bool state=false;
  bool isUserLiked;
  @override
  ...
  Future<bool> onLikeTapped(bool isLiked,DocumentSnapshot snapshot) async{
    print(isLiked);
    if(isLiked==true){
      await Firestore.instance.collection('collection').document('blouse').collection('designer_blouse').document(snapshot.data['uid']).updateData({'likedBy':FieldValue.arrayRemove([userId])});
    }
    if(isLiked==false){
      await Firestore.instance.collection('collection').document('blouse').collection('designer_blouse').document(snapshot.data['uid']).updateData({'likedBy':FieldValue.arrayUnion([userId])});
    }
