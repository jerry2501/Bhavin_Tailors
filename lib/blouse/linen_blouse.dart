...
class LinenBlouse extends StatefulWidget {

  @override
  _LinenBlouseState createState() => _LinenBlouseState();
}

class _LinenBlouseState extends State<LinenBlouse> {
  String userId;
  bool state=false;
  bool isUserLiked;
  @override
  ...
  Future<bool> onLikeTapped(bool isLiked,DocumentSnapshot snapshot) async{
    print(isLiked);
    if(isLiked==true){
      await Firestore.instance.collection('collection').document('blouse').collection('linen_blouse').document(snapshot.data['uid']).updateData({'likedBy':FieldValue.arrayRemove([userId])});
    }
    if(isLiked==false){
      await Firestore.instance.collection('collection').document('blouse').collection('linen_blouse').document(snapshot.data['uid']).updateData({'likedBy':FieldValue.arrayUnion([userId])});
    }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      StreamBuilder<QuerySnapshot>(
          stream: Firestore.instance.collection('collection').document('blouse').collection('linen_blouse').snapshots(),
